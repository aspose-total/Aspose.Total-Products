---
title: 通过 C++ 将 JSON 格式转换为 ODP
description: 在不使用 Microsoft PowerPoint 的情况下，在 C++ 中将 JSON 解析为 ODP
url: /zh/cpp/conversion/json-to-odp/
family: total
platformtag: cpp
feature: conversion
informat: JSON
...
{{< blocks/products/pf/feature-page-header h1="通过 C++ 将 JSON 格式转换为 ODP" h2="无需使用 Microsoft<sup>&reg;</sup> PowerPoint 即可将 JSON 解析为 ODP 的 C++ API" >}}

{{% blocks/products/pf/feature-page-summary %}}
您可以通过两个简单的步骤在任何 C++ 应用程序中将 JSON 转换为 ODP。首先，通过使用 [Aspose.Cells for C++](https://products.aspose.com/cells/cpp/)，您可以将 JSON 解析为 PPTX。之后，通过使用 [Aspose.Slides for C++](https://products.aspose.com/slides/cpp/)，您可以将 PPTX 转换为 ODP。这两个 API 都属于 [Aspose.Total for C++](https://products.aspose.com/total/cpp/) 包。 
{{% /blocks/products/pf/feature-page-summary  %}}
...
4. 使用[Save](https://reference.aspose.com/slides/cpp/class/aspose.slides.presentation#afcd59ec697bf05c10f78c3869de2ec9e)方法将文档保存为ODP格式
...
{{% blocks/products/pf/feature-page-section  h2="在 C++ 中将 JSON 格式转换为带有水印的 ODP" %}}
使用 API，您还可以将 JSON 转换为带水印的 ODP。为了给你的ODP文档添加水印，你可以先将JSON解析为PPTX并添加水印。为了添加水印，使用 [Presentation](https://reference.aspose.com/slides/cpp/class/aspose.slides.presentation) 类加载新创建的 PPTX 文件，获取第一张幻灯片，添加Rectangle 类型的 AutoShape，将 TextFrame 添加到 Rectangle，为文本框架创建 Paragraph 对象，为段落创建 Portion 对象，使用 set_Text() 添加水印，可以将文档保存到 ODP。
{{% blocks/products/pf/feature-page-code %}}
