...
6. Enregistrez le docxument dans POTM à l'aide de la méthode [save](https://apireference.aspose.com/slides/java/com.aspose.slides/Presentation#save-java.io.OutputStream-int-)
...
L'API vous permet également de convertir des docxuments DOCX protégés par mot de passe en POTM. Si votre docxument DOCX d'entrée est protégé par un mot de passe, vous ne pouvez pas le convertir au format POTM sans utiliser le mot de passe. Pour ouvrir un docxument crypté, vous pouvez définir le mot de passe correct dans l'objet LoadOptions et le transmettre au constructeur Docxument.  
{{% blocks/products/pf/feature-page-code %}}
