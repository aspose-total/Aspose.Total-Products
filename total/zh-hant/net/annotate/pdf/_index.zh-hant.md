---
title: 線上刪除 PDF 註釋或透過 .NET 管理註釋
description: 透過線上應用程式免費刪除 PDF 檔案中的註釋。用於管理 PDF 檔案註解的 C# .NET API 程式碼。
...
{{< blocks/products/pf/feature-page-header-widget h1="清除來自 PDF 線上文件的註解或透過 .NET 管理" h2="開發功能強大的基於 .NET 的 PDF 文件註解實用程式。列出用於透過 .NET 管理 PDF 檔案註解的程式碼。" >}}
...
{{% blocks/products/pf/agp/feature-section-col title="透過.NET 新增註釋" %}}

1. 將庫引用新增至 .NET 項目
1. 建立文檔類別對象
1. 使用 Pages.Add 和 Paragraphs.Add 新增頁面和段落
1. 使用 TextAnnotation 類別並定義註釋
1. 使用 Pages[1].Annotations.Add 新增註釋
1. 呼叫Save方法保存已註釋的文件
...
{{% blocks/products/pf/agp/code-block title=".NET 程式碼：在 PDF 檔案中加入註釋" offSpacer="" %}}
...
<h2>透過.NET開發PDF文件註解應用程式</h2>

需要開發 PDF 註釋應用程式或實用程式來提供回饋、提出建議或與其他人協作處理文件？透過 [Aspose.PDF for .NET](https://products.aspose.com/pdf/net/)（[Aspose.Total for .NET](https://products.aspose.com/total/net/) 的子 API），任何 .NET 開發人員都可以將上述 API 程式碼整合到其文件註解應用程式中。強大的.NET 程式庫允許對任何文件註解解決方案進行程式設計。而且它可以支援許多流行的格式，包括PDF格式。<br />

{{% /blocks/products/pf/feature-page-summary %}}

{{< blocks/products/pf/agp/feature-section >}}
{{% blocks/products/pf/agp/feature-section-col title="用於註解 PDF 檔案的 .NET 函式庫" %}}
