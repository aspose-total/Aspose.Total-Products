---
title: Java経由でPPSをDOCに変換する
description: MicrosoftWordやPowerPointを使用せずにPPSをDOCにエクスポートするJavaAPI
...
{{< blocks/products/pf/i18n/feature-page-header h1="Java経由でPPSをDOCに変換する" h2="任意のJavaJ2SE、J2EE、J2MEアプリケーション内でのPowerPointPPSからDOCへの変換のためのオンプレミスJavaAPI" >}}
...
APIを使用して、透かしを使用してPPSファイルからDOCへの変換を実行することもできます。 DOCドキュメントに透かしを追加するには、最初にPPSファイルをHTMLに変換し、それに透かしを追加します。透かしを追加するには、[Document](https://apireference.aspose.com/words/java/com.aspose.words/Document)クラスを使用して新しく作成したHTMLファイルをロードし、TextWatermarkOptionsのインスタンスを作成して設定します。そのプロパティ、Watermark.setTextメソッドを呼び出し、TextWatermarkOptionsの透かしテキストとオブジェクトを渡します。  
{{% blocks/products/pf/feature-page-code %}}
