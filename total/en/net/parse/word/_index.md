---
title: Extract Text and Images from Word File Online and using .NET
description: Online Word file parser app. .NET API C# code to extract images and text content from Word document.
family: total
platformtag: net
feature: Parse
informat: Word
...
hero:
  h1: Parse Word File Online as well as via .NET
  h2: Develop powerful .NET based Word document parser utility application. C# Code listed for Word document text extraction.
sections:
- layout: columns
  columns:
  - width: 6
    ...
    - type: steps
      role: online
      title: Parse Word Document via Online App
      items:
  ...
  - width: 6
    ...
    - type: steps
      role: api
      title: Parse Word File via .NET
      items:
      - Add library reference to .NET project
      - Load Word file using Document class object
      - Get all child nodes using GetChildNodes()
      - Use NodeType.Shape as parameter
      - Iterate through each node and save the image
      - For text extract, loop through each page
      - Call the ExtractPages method
      - Save extracted file to text using Node.ToString method
- layout: columns
  columns:
  - width: 12
    blocks:
    - type: code
      title: 'C# Code: Word Document Images Extraction'
      gist:
        user: aspose-com-gists
        id: ab98758eea66dd544d4a7964d10ec4fc
        file: extract-images-from-word-document.cs
- layout: columns
  columns:
  - width: 12
    blocks:
    - type: code
      title: 'C# Code: Word Document Text Extraction'
      gist:
        user: aspose-com-gists
        id: ab98758eea66dd544d4a7964d10ec4fc
        file: extract-text-from-word-document.cs
- layout: columns
  columns:
  - width: 12
    blocks:
    - type: markdown
      markdown: "#### \U0001F4D8 Microsoft Word Document (doc, docm, docx, dot, dotm, dotx, odt, rtf) Parsing Use Cases\n\n- \U0001F4C4 **Content Extraction**  \n  Extract text, tables, and images from Word documents for web display, search indexing, or text analysis.\n\n- \U0001F4F0 **Template Processing**  \n  Use `.dot`, `.dotx`, `.dotm` templates for auto-generating documents with dynamic data in workflows.\n\n- \U0001F501 **Automation Workflows**  \n  Automate parsing and content replacement in `.docm` or `.dotm` files that include embedded macros.\n\n- \U0001F9FE **Report Conversion**  \n  Convert `.doc`, `.odt`, or `.rtf` files to clean HTML, Markdown, or PDF for publishing or documentation.\n\n- \U0001F4DA **Digital Archiving**  \n  Archive legacy formats like `.doc` and `.rtf` by converting and indexing them for modern platforms."
      role: summary
- layout: columns
  columns:
  - width: 12
...
- layout: columns
  ...
  - width: 6
    blocks:
    - type: markdown
      title: .NET utility to process Word file for parser application
...
- type: faq
  ...
  - question: Is it safe to use the online app for parsing Word document?
  ...
  - question: What browser should to use App?
    answer: You can use any modern web browser like Google Chrome, Firefox, Opera, or Safari for online Word document parser. However, if you're developing a desktop application, we recommend using the Aspose.Total document processing API for efficient management.
- type: autogen_total
---
